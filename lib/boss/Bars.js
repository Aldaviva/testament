...
var Bars = module.exports = my.Class({
	...
	_getLeadingCols: function(fraction, totalCols){
	    return fill(Bars.BAR_CHARS[Bars.BAR_CHARS.length-1], Math.floor(fraction * totalCols));
	},

	_getTrailingCols: function(fraction, totalCols){
	    return fill(Bars.BAR_CHARS[0], Math.ceil((1-fraction) * totalCols) - 1);
	},
	...
	_getMiddleCol: function(fraction, totalCols){
	    if(fraction < 1){
	        ...
	        var charIndex     = Math.max(0, Math.min(Math.round(colFraction * Bars.BAR_CHARS.length-1), Bars.BAR_CHARS.length-1)) || 0;
	        return Bars.BAR_CHARS[charIndex];
	    } else {
	        return '';
	    }
	},
